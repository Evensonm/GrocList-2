const items = [
  ...
  function findItemById(id) {
    // const result = items.find((id) => items.id === id);
    // console.log(result);
  ...
  function capitalizeNames() {
    return items.map((item) => {
      ...
      const firstLetterOfItem = item.name.slice(0, 1);
      const capLetters = firstLetterOfItem.toUpperCase();
      console.log("letters", capLetters);
      const capNames = capLetters + item.name.slice(1);
  ...
  function calculateTotalInventory() {
    return (totalInventory = items.reduce(
      (next, item) => (next += item.inventory),
      0
    ));
  ...
  function logCartItems() {
    cart.forEach((cartItem) =>
      console.log(items.find((item) => item.id == cartItem))
    );
